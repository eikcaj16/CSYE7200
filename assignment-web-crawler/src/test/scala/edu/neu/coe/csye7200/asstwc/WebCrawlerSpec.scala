...
class WebCrawlerSpec extends AnyFlatSpec with should.Matchers with Futures with ScalaFutures with TryValues with Inside {
  ...
  val correctURL = "https://coe.northeastern.edu/people/xu-mofei/"
  ...
  "crawler(Seq[URL])" should s"succeed for $correctURL, depth 2" taggedAs Slow in {
    val args = List(s"$correctURL")
    ...
    whenReady(usf, timeout(Span(60, Seconds))) { s => Assertions.assert(s.length == 60) }
  }
